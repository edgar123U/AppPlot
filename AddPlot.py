...
authenticator = stauth.Authenticate(users, 'some_cookie_name', 'some_signature_key', cookie_expiry_days=30)
...
if authentication_status:
    ...
    if username not in st.session_state.users:
        st.session_state.users[username] = {'games': [], 'events': {'pass': [], 'shot': [], 'recovery': [], 'assist': [], 'duel': []}}
    ...
    def remove_game(game_name):
        if game_name in st.session_state.users[username]['games']:
            st.session_state.users[username]['games'].remove(game_name)
            if st.session_state.selected_game == game_name:
                st.session_state.selected_game = None
            st.session_state.users[username]['events'] = {'pass': [], 'shot': [], 'recovery': [], 'assist': [], 'duel': []}
    ...
    if st.button("Adicionar Jogo"):
        if game_name:
            st.session_state.users[username]['games'].append(game_name)
            st.success(f"Jogo '{game_name}' adicionado com sucesso!")

    # Seção para remover jogos
    st.sidebar.header("Remover Jogo")
    if st.session_state.users[username]['games']:
        game_to_remove = st.selectbox("Escolha um jogo para remover", st.session_state.users[username]['games'])
    ...
    selected_game = st.selectbox("Escolha um jogo", st.session_state.users[username]['games'])
    ...
    if selected_game:
        ...
        with tab1:
            ...
            if st.button("Adicionar Passe"):
                if end_x is not None and end_y is not None and player_name:
                    ...
                    st.session_state.users[username]['events']['pass'].append(event)
                    st.success("Passe adicionado com sucesso!")

            st.header("Remover Passe")
            if st.session_state.users[username]['events']['pass']:
                options = [f"Evento {i + 1}" for i in range(len(st.session_state.users[username]['events']['pass']))]
            ...
            if st.button("Remover Passe"):
                ...
                if index is not None:
                    st.session_state.users[username]['events']['pass'].pop(index)
                    st.success("Passe removido com sucesso!")

            # Desenhar o campo com eventos de passes
            fig = draw_pitch(st.session_state.users[username]['events']['pass'], 'pass')
            st.pyplot(fig)
        ...
        with tab2:
            ...
            xg = st.number_input("Probabilidade de Gol (xG)", min_value=0.0, max_value=1.0, step=0.01, key="shot_xg")
            ...
            if st.button("Adicionar Remate"):
                if player_name:
                    ...
                    st.session_state.users[username]['events']['shot'].append(event)
                    st.success("Remate adicionado com sucesso!")

            st.header("Remover Remate")
            if st.session_state.users[username]['events']['shot']:
                options = [f"Evento {i + 1}" for i in range(len(st.session_state.users[username]['events']['shot']))]
                selected_option = st.selectbox("Selecione um evento para apagar", options)
                if selected_option:
                    index = options.index(selected_option)
            ...
            if st.button("Remover Remate"):
                ...
                if index is not None:
                    st.session_state.users[username]['events']['shot'].pop(index)
                    st.success("Remate removido com sucesso!")

            # Desenhar o campo com eventos de remates
            fig = draw_pitch(st.session_state.users[username]['events']['shot'], 'shot')
            st.pyplot(fig)
        ...
        with tab3:
            ...
            if st.button("Adicionar Recuperação"):
                if player_name:
                    ...
                    st.session_state.users[username]['events']['recovery'].append(event)
                    st.success("Recuperação adicionada com sucesso!")

            st.header("Remover Recuperação")
            if st.session_state.users[username]['events']['recovery']:
                options = [f"Evento {i + 1}" for i in range(len(st.session_state.users[username]['events']['recovery']))]
                selected_option = st.selectbox("Selecione um evento para apagar", options)
                if selected_option:
                    index = options.index(selected_option)
            ...
            if st.button("Remover Recuperação"):
                ...
                if index is not None:
                    st.session_state.users[username]['events']['recovery'].pop(index)
                    st.success("Recuperação removida com sucesso!")

            # Desenhar o campo com eventos de recuperações
            fig = draw_pitch(st.session_state.users[username]['events']['recovery'], 'recovery')
            st.pyplot(fig)
        ...
        with tab4:
            ...
            if st.button("Adicionar Assistência"):
                if end_x is not None and end_y is not None and player_name:
                    ...
                    st.session_state.users[username]['events']['assist'].append(event)
                    st.success("Assistência adicionada com sucesso!")

            st.header("Remover Assistência")
            if st.session_state.users[username]['events']['assist']:
                options = [f"Evento {i + 1}" for i in range(len(st.session_state.users[username]['events']['assist']))]
                selected_option = st.selectbox("Selecione um evento para apagar", options)
                if selected_option:
                    index = options.index(selected_option)
            ...
            if st.button("Remover Assistência"):
                ...
                if index is not None:
                    st.session_state.users[username]['events']['assist'].pop(index)
                    st.success("Assistência removida com sucesso!")

            # Desenhar o campo com eventos de assistências
            fig = draw_pitch(st.session_state.users[username]['events']['assist'], 'assist')
            st.pyplot(fig)
        ...
        with tab5:
            ...
            if st.button("Adicionar Duelo Aéreo"):
                if player_name:
                    ...
                    st.session_state.users[username]['events']['duel'].append(event)
                    st.success("Duelo Aéreo adicionado com sucesso!")

            st.header("Remover Duelo Aéreo")
            if st.session_state.users[username]['events']['duel']:
                options = [f"Evento {i + 1}" for i in range(len(st.session_state.users[username]['events']['duel']))]
                selected_option = st.selectbox("Selecione um evento para apagar", options)
                if selected_option:
                    index = options.index(selected_option)
            ...
            if st.button("Remover Duelo Aéreo"):
                ...
                if index is not None:
                    st.session_state.users[username]['events']['duel'].pop(index)
                    st.success("Duelo Aéreo removido com sucesso!")

            # Desenhar o campo com eventos de duelos aéreos
            fig = draw_pitch(st.session_state.users[username]['events']['duel'], 'duel')
            st.pyplot(fig)

        # Mostrar todos os eventos adicionados
        st.write("Todos os eventos adicionados:")
        st.write(st.session_state.users[username]['events'])

        # Exportar para Excel
        if st.button("Exportar para Excel"):
            all_events_df = export_to_excel(st.session_state.users[username]['events'], selected_game)
            buffer = io.BytesIO()
